 ...
 */
public class GetVoteReportBySubDistrict {

    DatabaseProcess dp = new DatabaseProcess();
    private static final Logger log = Logger.getLogger(GetVoteReportBySubDistrict.class);

    public HashMap process(HashMap inputMap) {
        try {
            inputMap =  dp.GetVoterReportBySubDistrict(inputMap);
            
            if (inputMap.get(FieldParameter.resp_code).equals("0000")) {
                
                inputMap.put(RuleNameParameter.resp_desc, "Success Get All vote Report");
            } else {
                inputMap.put(RuleNameParameter.resp_code, "0001");
                inputMap.put(RuleNameParameter.resp_desc, "Failed Get All vote Report");
