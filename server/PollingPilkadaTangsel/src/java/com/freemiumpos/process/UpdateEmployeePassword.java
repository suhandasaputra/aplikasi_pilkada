    ...
    public HashMap process(HashMap inputMap) {
        try {
            ...
            if (result.get(FieldParameter.resp_code).equals(RuleNameParameter.respcodeSuccess)) {
                if (result.get(FieldParameter.userpassword).equals(inputMap.get(FieldParameter.oldPassword))) {

                    //save sebelum kirim ke MOST
//            HashMap responseMOST = new HashMap();
                    if (dp.updateEmployeePasword(inputMap)) {
                    ...
                    inputMap.put(RuleNameParameter.resp_desc, "Incorrect old Password. Please try again.");
//                        dp.updateNumfailpin(result.get(FieldParameter.userlogin).toString());
                }
            } else {
                inputMap.put(RuleNameParameter.resp_code, RuleNameParameter.respcodeLoginFailed);
                inputMap.put(RuleNameParameter.resp_desc, "Invalid user.");
            }
        } catch (Exception e) {
