 ...
 */
public class DeleteDiscountRules {
...
//            HashMap responseMOST = new HashMap();
            ...
            HashMap disc = dp.getDetailDiscountRules(inputMap);
            if (Integer.valueOf(inputMap.get(FieldParameter.userlevel).toString()) < Integer.valueOf(disc.get(FieldParameter.disc_level).toString())) {
                inputMap.put(RuleNameParameter.resp_code, "0001");
                inputMap.put(RuleNameParameter.resp_desc, "User level can't delete this rules.");
            } else {
                if (dp.DeleteDiscountRules(inputMap)) {
