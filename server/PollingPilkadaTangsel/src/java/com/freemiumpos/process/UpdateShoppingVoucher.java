 ...
 */
public class UpdateShoppingVoucher {
...
//            HashMap responseMOST = new HashMap();
            ...
            String vouchDetail[] = dp.getDetailShoppingVoucherForEdit(inputMap).split(",");
            if (vouchDetail[0].equals("0000")) {
                if (Integer.valueOf(inputMap.get(FieldParameter.userlevel).toString()) < Integer.valueOf(vouchDetail[1])) {
                    inputMap.put(RuleNameParameter.resp_code, "0001");
                    inputMap.put(RuleNameParameter.resp_desc, "Permission denied to update the voucher for this user.");
                } else {
                    if (inputMap.get(FieldParameter.outlet_id).toString().equals("*")) {
                        if (inputMap.get(FieldParameter.userlevel).toString().equals("4")) {
                            inputMap = dp.UpdateShoppingVoucher(inputMap);
                        } else {
                            inputMap.put(RuleNameParameter.resp_code, "0001");
                            inputMap.put(RuleNameParameter.resp_code, "Permission denied to update voucher all outlet for this user.");
                        }
                    } else {
