 ...
 */
public class GetVoteReportBySubDistrictVotingPlace {

    DatabaseProcess dp = new DatabaseProcess();
    private static final Logger log = Logger.getLogger(GetVoteReportBySubDistrictVotingPlace.class);

    public HashMap process(HashMap inputMap) {
        try {
            inputMap =  dp.GetVoterReportBySubDistrictVotingPlace(inputMap);
            
            if (inputMap.get(FieldParameter.resp_code).equals("0000")) {
                
                inputMap.put(RuleNameParameter.resp_desc, "Success Get vote Report");
            } else {
                inputMap.put(RuleNameParameter.resp_code, "0001");
                inputMap.put(RuleNameParameter.resp_desc, "Failed Get vote Report");
