...
// Add event listener for opening and closing details
        $('#table_active').on('click', 'td', function () {
            var $row = $(this).closest("tr");    // Find the row
            var id_votingplace_active = $row.find(".id_votingplace_active").text(); // Find the text
            ...
            var total_voter = $row.find(".total_voter").text(); // Find the text


            document.getElementById("id_votingplace_active_edit").value = id_votingplace_active;
            ...
            document.getElementById("total_voter_edit").value = total_voter;
            ...
            <div id="box2_edit_category">
                <div id="input_edit_category">
                    <form action="edit_cate" id="form_edit_category" method="post">
                        ...
                        <input type="text" id="id_votingplace_active_edit" hidden="" name="id_votingplace_active_edit" readonly="" required="" placeholder="id votingplace active" maxlength="30">
                        <input type="text" id="subdistrict_name_edit" name="subdistrict_name_edit" readonly="" required="" placeholder="subdistrict name" maxlength="50" style="background-color: aquamarine">
                        <input type="text" id="votingplace_name_edit" name="votingplace_name_edit" readonly="" required="" placeholder="votingplace name" maxlength="50" style="background-color: aquamarine">
                        <input type="text" id="total_voter_edit" name="total_voter_edit" required="" placeholder="total voter">
                    </form>
...
<script>
    function sub_del_prod() {
        var id_votingplace_active_edit = document.getElementById('id_votingplace_active_edit').value;
        ...
        if (id_votingplace_active_edit == "") {
            push.innerHTML = "<p>id votingplace active must be fill</p>";
        ...
        } else if (id_votingplace_active_edit != "") {
            var datjson14 = new Object();
            datjson14.id_votingplace_active_edit = id_votingplace_active_edit;
            $.ajax({
                ...
                success: function (response) {
                    if (response.status == 00) {
                        alert('success delete activation');
                        $('#table_active').DataTable().ajax.reload();
                    } else {
                        alert('failed delete activation');
    ...
    jQuery(document).ready(function ($) {
        $('#btn_save_edit_category').on('click', function (event) {
            ...
            var id_votingplace_active_edit = document.getElementById('id_votingplace_active_edit').value;
            var total_voter_edit = document.getElementById('total_voter_edit').value;

            var modal = document.getElementById("myModal_edit_cate");
            var span = document.getElementsByClassName("close_edit_cate")[0];
            var push = document.getElementById("push_text_edit_cate");
            if (id_votingplace_active_edit == "") {
                push.innerHTML = "<p>votingplace active must be Fill</p>";
                modal.style.display = "block";
                span.onclick = function () {
                    modal.style.display = "none";
                }
                ...
                } else if (total_voter_edit != "") {
                        var datjson15 = new Object();
                        datjson15.id_votingplace_active_edit = id_votingplace_active_edit;
                        datjson15.total_voter_edit = total_voter_edit;

                        $.ajax({
                            contentType: 'application/json',
                            dataType: "json",
                            url: "uas",
                            data: JSON.stringify(datjson15),
                            type: 'post',
                            success: function (response) {
                                if (response.status == 00) {
                                    alert('success update activation');
                                    $('#table_active').DataTable().ajax.reload();
                                } else {
                                    alert('failed update activation');
                                }
                            }
                        });
