...
// Add event listener for opening and closing details
        $('#table_bank').on('click', 'td', function () {
            var $row = $(this).closest("tr");    // Find the row
            var bankcode_edit = $row.find(".bankcode").text(); // Find the text
            var bankname_edit = $row.find(".bankname").text(); // Find the text

            document.getElementById("bankcode_edit").value = bankcode_edit;
            document.getElementById("bankname_edit").value = bankname_edit;
            ...
            <div id="box2_edit_category">
                <div id="input_edit_category">
                    <form action="edit_cate" id="form_edit_category" method="post">
                        ...
                        <input type="text" id="bankname_edit" name="bankname_edit" required="" placeholder="bank name" maxlength="50">

                    </form>
                    <div id="buttonon_edit_category">
                        <div id="btn_cancel_edit_category">Cancel</div>
                        <div id="btn_del_edit_category" onclick="sub_del_bank()">Delete</div>
...
<script>
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status == 00) {
                        ...
                        window.location.href = "lbk";
                    } else {
                        alert('failed delete bank');
    ...
    jQuery(document).ready(function ($) {
        $('#btn_save_edit_category').on('click', function (event) {
            var bankcode_edit = document.getElementById('bankcode_edit').value;
            var bankname_edit = document.getElementById('bankname_edit').value;
            ...
            if (bankcode_edit == "") {
                ...
                if (bankname_edit == "") {
                    push.innerHTML = "<p>bank name must be choose</p>";
                    modal.style.display = "block";
                    span.onclick = function () {
                        modal.style.display = "none";
                    }
