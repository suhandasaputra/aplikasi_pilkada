...
<div class="cd-popup-add-category" role="alert">
    <div class="cd-popup-add-category-container">
        <label id="label_add_category">Add Voting Place</label>
            ...
            <div id="box2_category">
                <div id="input_category">
                    <form>
                        ...
                        <input type="text" id="id_votingplace" name="id_votingplace" required="" placeholder="id voting place" maxlength="20">
                        <input type="text" id="votingplace_name" name="votingplace_name" required="" placeholder="votingplace name" maxlength="50">
                    </form>
                    <div id="buttonon">
...
<script>
    jQuery(document).ready(function ($) {
        $('#btn_save').on('click', function (event) {
            var id_votingplace = document.getElementById('id_votingplace').value;
            var votingplace_name = document.getElementById('votingplace_name').value;
            ...
            if (id_votingplace == "") {
                ...
                } else if (votingplace_name != "") {
                        var datjson10 = new Object();
                        datjson10.id_votingplace = id_votingplace;
                        datjson10.votingplace_name = votingplace_name;

                        $.ajax({
                            contentType: 'application/json',
                            dataType: "json",
                            url: "vps",
                            data: JSON.stringify(datjson10),
                            type: 'post',
                            success: function (response) {
                                if (response.status == 00) {
                                    alert('success add voting place');
                                    $('#table_conn').DataTable().ajax.reload();
                                } else {
                                    alert('failed add voting place');
